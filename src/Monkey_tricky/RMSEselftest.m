function RMSE = RMSEselftest(teamName,tr_test, direc_test)
% for self-test.
...
% Train Model
modelParameters = linearRegressionKalmanTraining(trainingData);
for tr=tr_test%:size(testData,1)
%     display(['Decoding block ',num2str(tr),' out of ',num2str(size(testData,1))]);
%     pause(0.001)
%     for direc=randperm(8) 
    for direc=direc_test
        decodedHandPos = [];
...
%             pastCurrentTrial.handposition = testData(tr,direc).handPos(1:2,1:t); 
            ...
            [decodedPosX, decodedPosY] = positionEstimatorKalman(pastCurrentTrial, modelParameters);
